...
HANDLE __utalCanvasWindowThread;
HANDLE __utalCanvasWindowMutex;
...
{
    WaitForSingleObject(__utalCanvasWindowMutex, INFINITE);
    
    bool success = true;
    __utalCanvasWindowMutex = CreateMutex( NULL, FALSE, NULL);
    if(__utalCanvasWindowMutex == NULL)
    {
        success = false;
    }
    ...
    ReleaseMutex(__utalCanvasWindowMutex);
    return success;
}
...
void UtalCanvasWindowDrawRectangle(char* data)
{
    ...
    UtalCanvasPlainRectangle* plainRectangle = (UtalCanvasPlainRectangle*)UtalCanvasUtilsFixRectangle(rectangle->x1, rectangle->y1, rectangle->x2, rectangle->y2); 
...
{
    if(__utalCanvasWindowThread != NULL)
        return;
    
    __utalCanvasWindowThread = CreateThread(NULL, 0, loop, NULL, 0, NULL);
}
